...
(async () => {
	...
	if (flags.password != undefined) {
		...
		o = await page.evaluate((o) => {
			...
			o.ipv4_address = document.querySelector('#ModemIP').innerText.trim();
		...
		//Make another syscall to get top info via ssh (for load parsing)
		o.top = shell.exec(`sshpass -p '${flags.password}' ssh -oKexAlgorithms=+diffie-hellman-group1-sha1 ${flags.username}@${flags.hostname} top -n1 -b`).stdout;
